...
public class Weapon : MonoBehaviour
{
    ...
    void RaycastShot()
    {
        ...
        if (Physics.Raycast(r, out hit, 1000.0f, ~(1 << 9), QueryTriggerInteraction.Ignore))
        {
            ...
            {
                int coins;
                MoleController.HitResult isHit = mole.GetComponent<MoleController>().Hit(out coins);

                // if hit the mole, show hummer and effect
                if (isHit == MoleController.HitResult.GOPHER)
                {

                    GameManager.instance.scorePoints(coins);
                }

                else if (isHit == MoleController.HitResult.BUNNY)
                {
